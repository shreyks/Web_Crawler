...
for j in range(1, 6):
    url = "https://www.amazon.com/best-sellers-books-Amazon/zgbs/books/ref=zg_bs_pg_2?_encoding=UTF8&pg=" + \
        str(j)
...
# soup2 = BeautifulSoup (str(soup.find_all('div', class_="a-section a-spacing-none p13n-asin")), "lxml")
    ...
    for i in soup.find_all('div', class_="a-section a-spacing-none p13n-asin"):
        ...
        if bookRating is None:
            # print "Rating --------- NA"
            tempData.append("Not Available")
        elif bookRating.get_text().strip() == "Prime":
            # print "Rating --------- NA"
            tempData.append("Not Available")
        else:
...
with open('./output/com_book.csv​', 'wb') as f:
    writer = csv.writer(f)
    for i in csvData:
        i = [s.encode('utf-8') for s in i]
        writer.writerows([i])
